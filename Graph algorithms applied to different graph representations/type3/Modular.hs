...
-}
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f xs = foldr (\x acc -> (take x xs ++ (f (xs!!x)) : drop (x+1) xs) : acc) [] [0..(length xs - 1)]
...
-}
partitions :: [a] -> [[[a]]]
partitions xs = reverse $ map reverse $ prt xs where
    prt xs = if length xs == 0 then [[]] else [ y | ys <- prt (tail xs), y <- aux_prt (head xs) ys] where
        aux_prt a xs = if length xs == 0 then [[[a]]] else ((a:(head xs)):(tail xs)) : map ((head xs):) (aux_prt a (tail xs))
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = if S.size (S.map (\n -> S.filter (\n -> if S.member n set then False else True) $ outNeighbors n graph) set) == 1
                        && S.size (S.map (\n -> S.filter (\n -> if S.member n set then False else True) $ inNeighbors n graph) set) == 1 
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = if S.member True modPart && S.size modPart == 1 then True else False where
    modPart = S.map (\p -> isModule p graph) partition
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = minimumBy (compare `on` S.size) $ filter (\p -> isModularPartition p graph && S.size p /= 1) partitions
-- size != 1 ca sa nu alegem tot graful. Alegem module ale acestuia.
