...
{-
Gândiți-vă cum ați putea generaliza această interfață de lucru cu grafuri în Haskell:

   1. Cum ați defini o clasă care să surprindă conceptul de graf orientat? Ați parametriza-o cu tipul concret (de exemplu, AlgebraicGraph a) sau cu constructorul de tip în sine (de exemplu, AlgebraicGraph)?
   2. Cum ar arăta tipul funcției nodes dacă ar fi definită în interiorul acestei clase?

1. As parametriza-o cu tipul concret:
class AlgebraicGraph g where --g constructor de tip
    Node :: a -> g a
    Overlay :: (g a) -> (g b) -> g (g a) (g b)
    Connect :: (g a) -> (g b) -> g (g a) (g b)

2. Iar nodes (in interiorul clasei de mai sus) ar fi:
    nodes :: Ord a => (g a) -> a -> S.Set a
-}
...
splitNode node targets = extend (\n -> if n == node then foldl (\acc x -> Overlay acc x) Empty (map (\x -> (Node x)) targets) else Node n)
...
removeNode node graph = filterGraph (\n -> if n /= node then True else False) graph