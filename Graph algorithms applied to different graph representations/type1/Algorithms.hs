...
auxSearch f node graph visited struct = 
    if struct == []
        then []
        else
            if S.member node visited
                then if tail struct == []
                    then []
                    else [] ++ (auxSearch f (head $ tail struct) graph visited (tail struct))
                else 
                    let newStruct = f (tail struct) $ S.toList $ S.filter (\x -> S.notMember x visited) $ outNeighbors node graph
                    in node : (auxSearch f (head newStruct) graph (S.insert node visited) newStruct)

search f node graph = auxSearch f node graph S.empty [node]
...
bfs n graph = search (\l1 l2 -> concat [l1, l2]) n graph
...
dfs n graph = search (\l1 l2 -> concat[l2, reverse(l1)]) n graph
...
countIntermediate from to graph = if and [countBFS > 0, countDFS > 0, S.member to $ S.fromList $ tail $ bfs from graph] then Just (countBFS, countDFS) else Nothing
    where 
        countBFS = length $ fst $ span (/= to) $ tail $ bfs from graph
        countDFS = length $ fst $ span (/= to) $ tail $ dfs from graph