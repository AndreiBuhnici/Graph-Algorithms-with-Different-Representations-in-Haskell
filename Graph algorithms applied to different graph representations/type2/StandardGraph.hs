...
nodes (Graph nodesG edgesG) = nodesG
...
edges (Graph nodesG edgesG) = edgesG
...
removeNode node graph =  Graph (S.delete node ln) (S.fromList $ filter (\x -> case () of
      _ | fst x == node -> False
        | snd x == node -> False
        | otherwise -> True
    ) $ S.toList le)
...
splitNode old news graph = Graph (S.union (nodes g) (S.fromList news))
                            (S.union (edges g) $
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = Graph (if g == graph then nodes g else S.insert node (nodes g))
                              (S.union (edges g) $
